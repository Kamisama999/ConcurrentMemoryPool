...
void BenchmarkMalloc(size_t ntimes, size_t nworks, size_t rounds)
{
	...
	printf("%u个线程并发执行%u轮次，每轮malloc %u次: 花费：",
		nworks, rounds, ntimes);
	cout << malloc_costtime << " ms" << endl;
	printf("%u个线程并发执行%u轮次，每轮free %u次: 花费：",
		nworks, rounds, ntimes);
...
void BenchmarkConcurrentMalloc(size_t ntimes, size_t nworks, size_t rounds)
{
	...
	printf("%u个线程并发执行%u轮次，每轮concurrent alloc %u次: 花费：",
		nworks, rounds, ntimes);

	cout << malloc_costtime << " ms" << endl;
	printf("%u个线程并发执行%u轮次，每轮concurrent dealloc %u次: 花费：",
		nworks, rounds, ntimes);
	cout << free_costtime << " ms" << endl;
